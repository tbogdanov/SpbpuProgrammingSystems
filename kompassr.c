...
#define DL_OBJTEXT 90                             /*длина об'ектн. текста   */
  ...
  if ( PRNMET == 'Y' )                            //если псевдооп.DC помеч.,
   {                                              //то:                     
    ...
    switch (TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0])
        {
            case 'F': T_SYM[ITSYM].DLSYM = 4; break; //  уст.длину симв. =  4 
            case 'H': T_SYM[ITSYM].DLSYM = 2; break; //  или 2 для полуслова 
            default: return 1;
        }
...
int SDC()                                         /*подпр.обр.пс.опер.DC    */
 {
  ...
  if (!memcmp(TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, "H'", 2))
  {
...
CONT3:
 ...
 T_MOP[4].BXPROG = SRX;
 T_MOP[5].BXPROG = SRX;
 T_MOP[6].BXPROG = SRR;                           /*на подпрограммы обраб-ки*/
