...
#define DL_OBJTEXT 180                             /*длина об'ектн. текста   */
...
int FDC()                                         /*подпр.обр.пс.опер.DC    */
 {
  ...
  if ( PRNMET == 'Y' )                            //если псевдооп.DC помеч.,
   {                                              //то:                     
    ...
    {
        case 'F': symb_length = 4; break; //  уст.длину симв. =  4 
        case 'H': symb_length = 2; break; //  или 2 для полуслова 
        default: return 1;
    }
        
    T_SYM[ITSYM].PRPER = 'R';                   //  призн.перемест.='R' 
    if ( CHADR % symb_length )                            //  и, если CHADR не указ
    {                                           //  на границу слова, то: 
        CHADR = (CHADR / symb_length + 1) * symb_length;             //   уст.CHADR на гр.сл. и
        T_SYM[ITSYM].ZNSYM = CHADR;             //   запомн. в табл.симв. 
    }
    CHADR = CHADR + symb_length;
...
int SDC()                                         /*подпр.обр.пс.опер.DC    */
 {
  ...
  RR.OP_RR.OP   = 0;        // Добавлено: занулим и RR для случая с полусловами
  RR.OP_RR.R1R2 = 0;
...
CONT3:
 ...
 T_MOP[4].BXPROG = SRX;
 T_MOP[5].BXPROG = SRX;
 T_MOP[6].BXPROG = SRX;                           /*на подпрограммы обраб-ки*/
 T_MOP[7].BXPROG = SRX;                           /*команд АССЕМБЛЕРА при   */
 T_MOP[8].BXPROG = SRX;                           /*втором просмотре        */
 T_MOP[9].BXPROG = SRX;
 T_MOP[10].BXPROG = SRX;
 T_MOP[11].BXPROG = SRX;
