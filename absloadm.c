...
#define  NOBJ   90                                /*разм.масс.об'ектных карт*/
...
long get_addr_by_bxd(long base, long index, long shift)
{
    /* Выдаёт адрес участка памяти по номеру базового
     * и индексного регистров и сдвигу. Выделено как
     * отдельный метод, потому что используется повсеместно */
    return VR[base] + VR[index] + shift;
}
 
int get_value_by_addr(long base, long index, long shift)
{
    /* Выдаёт значение в участке памяти по номеру базового
     * и индексного регистров и сдвигу. Используется в P_LH, P_AH, P_SH, P_CH. */
    int sm;
    
    ADDR = get_addr_by_bxd(base, index, shift);
    
    sm = (int) (ADDR - I);
    
    return OBLZ[BAS_IND + CUR_IND + sm] * 0x100 +   /*второго операнда к виду,*/
        OBLZ[BAS_IND + CUR_IND + sm + 1];             /*качестве 1-го операнда  */
}
 
int BC_check(int mask)
{
    /* Здесь проверяется соответствие значения в регистре CC
     * (после команды CH) маске (первому операнду BC).
     * Возвращает 1, если соответствие обнаруживается. Тогда
     * BC инициирует переход к адресу, соотв. метке, указанной
     * во втором операнде.
     */
    ...
    {
        case 0 /* равно */: 
            if (mask == 8 || mask == 11 || mask == 13)
                return 1;
            break;
        case 1 /* меньше */:
            if (mask == 4 || mask == 7 || mask == 13)
                return 1;
            break;
        case 2 /* больше */:
            if (mask == 2 || mask==7 || mask == 11)
                return 1;
            break;
    }
...
int P_STH()
{
    int sm,i;
    char bytes[2];
    ADDR = get_addr_by_bxd(B, X, D);
    sm = (int) (ADDR -I);
    bytes[0] = ((VR[R1] % 0x10000L) - (VR[R1]%0x100)) / 0x100;
    bytes[1] = (VR[R1] % 0x10000L) % 0x100;
    for (i=0; i<2; i++) OBLZ[BAS_IND + CUR_IND + sm + i] = bytes[i];
...
int P_CH()
{
    ...
    long right = get_value_by_addr(B, X, D);        
    
    if (left == right) CC = 0;
    else if (left < right) CC = 1;
    else CC = 2;
...
int P_BC()
{
    ...
    if (BC_check(mask)) // условие соблюдено, переход
    {
        ADDR = get_addr_by_bxd(B, X, D);
        sm = (int) (ADDR - I);
        CUR_IND = CUR_IND + sm;
...
//нижнее поле     
  ...
  waddstr(wmargenta, "\"PgUp\",\"PgDn\",\"Up\",\"Down\"->scroll dump; \"Enter\"->step next");
...
//прокрутка окна
  ...
  for ( i = 0; i < 16; i++)
  {
    ...
    wprintw(wblue, "%d:", i);
    if (VR[i] < 0)
        wprintw(wblue, "%.8lX", VR[i] >> 8);
    else
        wprintw(wblue, "%.8lX", VR[i]);
    
  }      
  waddstr(wblue, "CC: ");
  wprintw(wblue, "%.8lX", CC);
  
  wrefresh(wblue);			//вывод на экран		  
  wclear(wblue);			//очистка окна регистров
  wind();   
      
  waddstr(wcyan, "Ready to execute command addr ");
  wprintw(wcyan, "%.08lX", I - T_MOP[k].DLOP);
  waddstr(wcyan, "\n");    				
  wrefresh(wcyan);
  wclear(wcyan);
